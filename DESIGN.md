...
## 10. Button craft + demo cleanup (Aug 19, night)

- **Buttons customized (`.btn-brutal`):** hover now slides a fill layer up from the bottom edge (ink on outline buttons, clay on solid ones, 0.4s expo), the label and icons ride above it, the trailing arrow glyph nudges forward, and pressing snaps the fill in fast. Focus-visible triggers the same fill. Applied globally — nav GET STARTED, hero CTAs, roll panels, contact, process, footer, mobile menu — with no call-site changes.
- **Omni press-to-talk demo: REMOVED** (owner's call — dead weight, it was a simulated transcript). `VoiceDemo.tsx` deleted, the `.voice-bar` CSS keyframes removed, and its e2e test dropped. The Omni project itself stays featured in the horizontal roll; the Expertise section is now just the Swiss capability table.
