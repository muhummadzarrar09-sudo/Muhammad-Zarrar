...
/**
 * Lenis smooth scroll — module-scoped singleton, back by owner's request
 * (removed once, missed, reinstalled to drive the project roll + velocity
 * marquees). (Deliberately NOT exposed on `window` — no globals here.)
 *
 * Disabled entirely for `prefers-reduced-motion` users: they get native
 * scrolling, and every consumer falls back gracefully.
 */
