      ...
      <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8">
        <div className="flex flex-col justify-between gap-10 md:flex-row">
          <div className="max-w-sm">
            <button type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="font-display text-3xl font-light tracking-tightest">
            ...
            <button type="button" onClick={() => navigate("/")} data-hover className="link-underline mt-4 w-fit text-xs text-muted transition-colors hover:text-ink">
          ...
          <div className="flex flex-wrap gap-x-14 gap-y-6">
            <div className="flex flex-col gap-2.5">
              ...
              {bizLinks.map((l) => (
                <button type="button" key={l.id} onClick={() => go(l.id)} className="link-underline w-fit text-sm text-ink-soft">
                  {l.label}
                </button>
              ))}
            </div>
            <div className="flex flex-col gap-2.5">
              ...
              <a href={biz.whatsappCta} target="_blank" rel="noopener noreferrer" className="link-underline w-fit text-sm text-ink-soft">
                WhatsApp ↗
              </a>
              <a href={`mailto:${biz.email}`} className="link-underline w-fit text-sm text-ink-soft">
                Email ↗
              </a>
              <a href={biz.github} target="_blank" rel="noopener noreferrer" className="link-underline w-fit text-sm text-ink-soft">
                GitHub ↗
              </a>
            </div>
          </div>
