...
/* ------------------------------------------------------------------ */
/* Envelope — paper, wax, and a form folded inside                     */
/*                                                                     */
/* Story: closed → HOVER (the flap swings open and the letter — a      */
/* miniature of the form — slides up so you can see what's inside)     */
/* → CLICK (seal breaks, the letter is drawn out toward you and        */
/* settles into the FULL form). After send it folds back in and the    */
/* seal stamps on: "Draft sealed".                                     */
/* ------------------------------------------------------------------ */

const letterVariants = {
  closed: { y: 0, scale: 1, rotate: 0 },
  // Hover — the letter rises in the pocket so the mini-form peeks out
  hover: { y: -58, transition: { type: "spring" as const, stiffness: 260, damping: 22 } },
  opening: { y: -46, transition: { duration: 0.34, delay: 0.1, ease: EASE } },
  // Drawn out — comes toward you before it becomes the form
  lifted: {
    y: -150,
    scale: 1.18,
    rotate: -2,
...
const flapVariants = {
  closed: { rotateX: 0 },
  // Hover — the flap swings right open, inviting the click
  hover: { rotateX: -158, transition: { type: "spring" as const, stiffness: 160, damping: 18 } },
...
const sealVariants = {
  ...
  hover: { scale: 1.08, rotate: -4, transition: { type: "spring" as const, stiffness: 320, damping: 11 } },
...
/** The letter tucked inside — a miniature of the contact form. */
function LetterInside() {
  return (
    <div className="paper-finish absolute inset-x-8 top-1 z-10 rounded-t-md border border-line bg-canvas shadow-[0_10px_24px_-14px_rgba(0,0,0,0.35)]">
      <div className="px-4 pb-5 pt-3">
        <div className="text-center font-display text-[11px] italic text-ink-soft/80">
          hey — got a minute?
        </div>
        {/* The mini form — name, email, a message block, a send chip */}
        <div className="mt-2.5 space-y-1.5" aria-hidden="true">
          <div className="h-2 rounded-[3px] border border-line bg-surface-2" />
          <div className="h-2 rounded-[3px] border border-line bg-surface-2" />
          <div className="space-y-1 rounded-[3px] border border-line bg-surface-2 p-1.5">
            <div className="h-px bg-line-strong/50" />
            <div className="h-px w-5/6 bg-line/80" />
            <div className="h-px w-4/6 bg-line/70" />
          </div>
          <div className="flex items-center justify-between">
            <span className="font-mono text-[7px] uppercase tracking-[0.18em] text-faint">
              your note
            </span>
            <span className="h-2.5 w-9 rounded-[3px] bg-clay-deep/90" />
          </div>
        </div>
      </div>
    </div>
...
function EnvelopeBody() {
  return (
    <div className="relative w-full">
      {/* The letter — rises in the pocket on hover, drawn out on click */}
      <motion.div variants={letterVariants} className="h-0">
        <LetterInside />
      </motion.div>

      {/* Envelope shell */}
      <motion.div variants={bodyGroupVariants} className="relative z-20" style={{ perspective: "1100px" }}>
        {/* Envelope body — papery */}
        <div className="paper-finish relative aspect-[8/5] w-full overflow-hidden rounded-2xl border border-line-strong bg-surface lift">
          {/* Back seams */}
          <svg
        ...
        <motion.div
          ...
          className="paper-finish absolute inset-x-0 top-0 z-20 h-[56%] rounded-t-2xl bg-surface-2"
        >
...
export default function Contact() {
  ...
  /* Hover already opened the flap — click breaks the seal and draws
     the letter out; it settles into the full form. */
  const openEnvelope = () => {
    ...
    setPhase("opening");
    // Flap swings open + seal breaks first…
    timers.current.push(window.setTimeout(() => setPhase("lifted"), 420));
    // …the letter is drawn out, then settles into the form.
  ...
  /* Put the cursor on the name field once the letter has settled */
  useEffect(() => {
    ...
    <section id="contact" className="mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-32">
      <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <Reveal delay={0.1} className="relative">
          {/* perspective so the letter visibly unfolds toward you */}
            ...
            <AnimatePresence mode="wait" initial={false}>
              {status === "sent" ? (
                ...
                >
                  ...
                  <motion.div initial="closed" animate="sealed" className="mx-auto w-full max-w-[480px]">
                ...
                <motion.div
                  key="form"
                  /* The drawn-out letter settles down into place as the form */
                ...
                >
                  <form
                    onSubmit={submit}
                    className="paper-finish notebook-page relative space-y-5 rounded-2xl border border-line-strong bg-surface p-6 pt-9 sm:p-8 sm:pt-10"
                    noValidate
                  >
                    <Staple />
                ...
                <motion.div
                  key="envelope"
                  /* The letter keeps coming toward the camera as we cut away */
                ...
                >
                  ...
                  >
                    {/* The whole envelope lifts slightly while hovered */}
                  ...
                  <div className="mt-5 text-center font-mono text-[10px] uppercase tracking-[0.28em] text-faint">
                    {phase === "opening"
                      ...
                      : phase === "lifted"
                        ? "— for you —"
                        : hovering
                          ? "— it&apos;s all inside — click —"
                          : "— the form is inside — hover to peek —"}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
