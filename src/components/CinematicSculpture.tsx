...
function Sculpture({ mouse }: { mouse: { x: number; y: number } }) {
  ...
  useFrame((state) => {
    const t = state.clock.getElapsedTime() * 0.013;
    if (outer.current) {
      outer.current.rotation.y = t * 0.15 + mouse.x * 0.72;
      outer.current.rotation.x = Math.sin(t * 0.06) * 0.028 + mouse.y * 0.5;
    }
    if (core.current) core.current.rotation.y = -t * 0.65 + mouse.x * -0.22;
    if (ring.current) ring.current.rotation.z = t * 0.065;
    if (accent.current) accent.current.rotation.y = t * 1.1 + mouse.x * 0.4;
    if (innerRing.current) innerRing.current.rotation.z = -t * 0.36;
    if (secondary.current) secondary.current.rotation.y = t * 0.78 + mouse.x * 0.25;
  });

  return (
    <group ref={outer}>
      <mesh>
        <torusKnotGeometry args={[1.05, 0.42, 96, 16, 2, 7]} />
        <meshPhongMaterial color="#ff6a3d" emissive="#1c1914" shininess={82} specular="#f4f1ea" flatShading={false} />
      </mesh>
      <mesh position={[0.6, -0.3, 0.4]} rotation={[0.8, 1.6, 0.3]}>
        <torusKnotGeometry args={[0.68, 0.29, 64, 12, 3, 5]} />
        <meshPhongMaterial color="#c08532" emissive="#1c1914" shininess={62} specular="#f4f1ea" transparent opacity={0.85} flatShading={false} />
      </mesh>
      <group ref={core}>
        <mesh position={[0, 1.12, 0]}>
          <sphereGeometry args={[0.81, 24, 18]} />
          <meshPhongMaterial color="#f4f1ea" shininess={125} specular="#ff4d17" flatShading={false} />
        </mesh>
      </group>
      <group ref={ring}>
        <mesh position={[0, 0.1, 0]} rotation={[1.6, 0, 0]}>
          <torusGeometry args={[2.42, 0.016, 12, 48]} />
...
export function CinematicSculpture() {
  ...
  return (
    ...
    >
      <Canvas camera={{ position: [0, 0.1, 8.6], fov: 29 }} gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }}>
...
export default CinematicSculpture;
